...
def index():
    ...
    list_name_img = [
        name_img
        for name_img in elements_in_portfolios
        if os.path.isfile(os.path.join(path_portfolios, name_img))
    ]
    paths = [
        url_for("static", filename=os.path.join("portfolios", name_img))
    ...
    list_name_img = [
        name_img
        for name_img in elements_in_partenaires
        if os.path.isfile(os.path.join(path_partenaires, name_img))
    ]
    paths = [
        url_for("static", filename=os.path.join("logo_partenaires", name_img))
        for name_img in list_name_img
    ]
    ...
    return render_template(
        "index.html", images_portfolios=results, logo_partenaires=paths
    )
...
def contact():
    ...
    requests.post(
        "https://api.mailgun.net/v3/mail.4-you.fr/messages",
